...
const CatalogMenu = ({
  ...
  // Local states for color, mainSize and category selections and range value
  const [tempColor, setTempColor] = useState(selectedColor);
  const [tempSize, setTempSize] = useState(selectedSize); // Added for mainSize

  // Handlers for color, mainSize and category selection, applying color selection and range change
  ...
  const handleApplyColorAndSizeSelection = () => {
    // Modified to apply both color and mainSize
      ...
      <div className={containerClassNames}>
        ...
        <div className="mt-4 overflow-y-scroll scroll whitespace-nowrap scroll-smooth">
          ...
          <ul className="flex flex-col gap-y-2 pl-8 mt-4 text-[#ABABAB]">
            <Link to="/Jackets">
              <p className="text-base cursor-pointer">Jackets</p>
            </Link>
            <Link to="/Stringers">
              <p className="text-base cursor-pointer">Stringers</p>
            </Link>
            <Link to="/Lifting">
              <p className="text-base cursor-pointer">Lifting</p>
            </Link>
            <Link to="/Shorts">
